...
export function BookmarksPanel({ editor, onClose }: BookmarksPanelProps) {
  ...
  const jumpTo = (pos: number) => {
    ...
    (domResult.node as HTMLElement | null)?.scrollIntoView?.({ behavior: 'smooth', block: 'center' });
  };

  return (
    <div className="flex min-w-56 flex-col rounded-xl border border-border bg-popover shadow-lg">
        ...
        {bookmarks.length === 0 ? (
          ...
          bookmarks.map((bm) => (
            ...
            >
              ...
              <span className="truncate text-foreground">{bm.label}</span>
            </button>
          ))
        )}
      </div>
